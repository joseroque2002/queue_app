...
class _InformationFormScreenState extends State<InformationFormScreen>
  ...
  String _userType = 'Student'; // Student, Graduated, or External
  ...
  Future<void> _loadData() async {
    ...
    final departments = _departmentService.getDepartmentCodes();

    // Load purposes
    await _purposeService.initializeDefaultPurposes();
    final purposes = _purposeService.getPurposeNames();
    ...
    if (mounted) {
      setState(() {
        ...
        _purposes = purposes;
        if (_purposes.isNotEmpty && _selectedPurpose == null) {
          _selectedPurpose = _purposes.first;
        }
      });

      // Initialize courses after departments are loaded
      _initializeCourses();
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // Use "Others" as purpose and store custom text in notes if "Others" is selected
      final purposeValue = _isOthersPurpose ? 'Others' : _selectedPurpose!;
      final notesValue = _isOthersPurpose && _customPurposeController.text.isNotEmpty
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    children: [
                      ...
                      _buildDetailRow(
                        'User Type',
                        entry.userType == 'Graduated' && entry.graduationYear != null
                            ? '${entry.userType} (${entry.graduationYear})'
                            : entry.userType,
                        context,
                      ),
  ...
  void _resetForm() {
    ...
    _selectedPurpose = _purposes.isNotEmpty ? _purposes.first : null;
                      ...
                      child: Form(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // Course dropdown (shown for all departments) - REQUIRED (Hidden for External)
                            if (_userType != 'External')
                              DropdownButtonFormField<String>(
                            ...
                            DropdownButtonFormField<String>(
                              ...
                              onChanged: (value) {
                                setState(() {
                                  _userType = value!;
                                  // Clear graduation year if switching to Student or External
                                  if (_userType == 'Student' || _userType == 'External') {
                                    _graduationYear = null;
                                  }
                                  // Clear course if switching to External
                                  if (_userType == 'External') {
                                    _selectedCourse = null;
                                  }
                                });
